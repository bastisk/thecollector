...
function LoginCtrl($scope, $state, UserList, $http) {
    ...
    $http.get("http://basti-sk.com/php/users.php").success(function(result) {
        ...
        {
            if($scope.users[i].PW_Hash == UserPW_Hash && $scope.users[i].Name == UserName) $state.go('tabs.movies');
        }
    });
    };
}
...
function Movielist($http, $log){
    this.getMovies = function($scope) {
        $scope.movies = [
            {title: "Film A", genre: "Horror", watched: 0, voting: 1},
            {title: "Film B", genre: "SciFi", watched: 4, voting: 3},
            {title: "Film C", genre: "Documentation", watched: 0, voting: 6},
            {title: "Film D", genre: "Action", watched: 2, voting: 2},
            {title: "Film E", genre: "Action", watched: 2, voting: 5},
            {title: "Film F", genre: "Horror", watched: 6, voting: 2}
        ];
    }
}
